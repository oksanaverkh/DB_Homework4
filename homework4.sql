...
SELECT mark, color, count(color) AS number_of_cars FROM auto
WHERE mark = 'BMW' or mark = 'LADA'
GROUP by mark, color
ORDER BY mark;

-- 2. Вывести на экран марку авто(количество) и количество AUTO не этой марки
SELECT a1.mark, count(a1.mark) AS number_of_cars, 
(SELECT count(*) from auto)-count(a1.mark) AS number_of_another_cars 
FROM auto a1
GROUP by mark;
...
UNION ALL  
SELECT test_b.id FROM test_b) 
AS t
GROUP BY id
HAVING count(id)=1) 
AS id_table
ON test_a.id = id_table.id; 
